    ...
    try:
        ...
        for model in models:
            ...
            val_scores = cross_val_score(
                model_instance, X_train, y_train, scoring='roc_auc', cv=stratified_kfold)
        ...
        val_df = pd.DataFrame(list(models_val_scores.items()), columns=[
                              'Model', 'Average Val Score'])
        train_df = pd.DataFrame(list(models_train_scores.items()), columns=[
                                'Model', 'Train Score'])
        ...
        val_bars = plt.bar(x - width/2, eval_df['Average Val Score'],
                           width, label='Average Validation Score', color='skyblue')
        train_bars = plt.bar(
            x + width/2, eval_df['Train Score'], width, label='Train Score', color='orange')
...
def evaluate_classifier(y_true, y_pred):
    ...
    This function computes and displays a classification report, confusion matrix, and ROC-AUC curve for a binary classifier's
    predictions. It provides insights into the classifier's performance, precision, recall, and discrimination capability.

    Args:
        y_true (array-like): The true binary target labels.
        y_pred (array-like): The predicted binary labels generated by the classifier.

    Returns:
        None

    Raises:
        CustomException: If an error occurs while evaluating the classifier.
    ...
    try:
        ...
        plt.plot([0, 1], [0, 1], linestyle='--',
                 color='gray')  # Random guessing line
...
def plot_feature_importances(model, data):
    ...
    try:
        ...
        plt.bar(range(len(importances)), sorted_importances,
                tick_label=sorted_feature_names, color=color_sequence)
...
def get_threshold_metrics(precision, recall, threshold, target_metric, target_metric_value):
    ...
    try:
        if target_metric == 'recall':
            ...
            target_value_index = np.where(
                recall_array[:-1] == target_metric_value)[0][0]

        else:
            precision_array = np.asarray(precision)
            target_value_index = np.where(
                precision_array[:-1] == target_metric_value)[0][0]
